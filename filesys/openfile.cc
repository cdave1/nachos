...
OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    //fileSystem->WriteLock();

    // Allocate some new space.
    int sector = hdr->GetSector();
    if (sector != FreeMapSector)
    {
      fileSystem->WriteLock();
      bool res = fileSystem->Allocate(hdr, numBytes);
      if (res == FALSE) {
	fileSystem->WriteUnlock();
	return 0;
      }
    }
    #endif
    ...
    DEBUG('f', "Writing %d sectors from sector %d to %d.\n", 	
	  numSectors, firstSector, lastSector);  
