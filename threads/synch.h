...
class Lock {
  ...
  private:
    ...
    Thread *lockOwner;                      // remember who acquired the lock
    Semaphore *sem;                    // use semaphore for the actual lock
};
...
class ReadWriteLock {
  ...
  private:
    ...
    int readerCount;
    Thread *writeOwner; // debug.
    enum status { FREE=0, READ, WRITE, } lockStatus;
}; 
